...
(function (PathDisplayFormat) {
  ...
  PathDisplayFormat[(PathDisplayFormat["FolderWithFilename"] = 3)] =
    "FolderWithFilename";
  PathDisplayFormat[(PathDisplayFormat["FolderPathFilenameOptional"] = 4)] =
...
function isSystemSuggestion(obj) {
  return (
    isFileSuggestion(obj) ||
    isUnresolvedSuggestion(obj) ||
    isAliasSuggestion(obj)
  );
}
...
function generateMarkdownLink(fileManager, vault, sugg, sourcePath, options) {
  ...
  options = Object.assign(
    { useBasenameAsAlias: true, useHeadingAsAlias: true },
    options,
  );
  if (sugg) {
    ...
    switch (sugg.type) {
      ...
      case SuggestionType.SymbolList: {
        ...
        if (isHeadingCache(symbol)) {
          ...
          const isExternalLink = new RegExp(/^\[(.*?)\]\((.+?)\)/).test(
            symbol.original,
          );
          if (isExternalLink) {
    ...
    if (destFile && !linkStr) {
      ...
      linkStr = fileManager.generateMarkdownLink(
        destFile,
        sourcePath,
        subpath,
        alias,
...
class SwitcherPlusSettings {
  static get defaults() {
    ...
    return {
      ...
      enabledRibbonCommands: [Mode[Mode.HeadingsList], Mode[Mode.SymbolList]],
...
class GeneralSettingsTabSection extends SettingsTabSection {
  ...
  showPathDisplayFormat(containerEl, config) {
    ...
    options[PathDisplayFormat.FolderWithFilename.toString()] =
  ...
  showEnabledRibbonCommands(containerEl, config) {
    ...
    this.createSetting(containerEl, "Show ribbon icons", desc).addTextArea(
      (textArea) => {
  ...
  showOverrideMobileLauncher(containerEl, config) {
    ...
    this.addDropdownSetting(
      ...
      (rawValue, config) => {
        ...
        config.save();
        this.mainSettingsTab.plugin.updateMobileLauncherButtonOverride(
          isEnabled,
        );
      },
    );
...
class VaultListSettingsTabSection extends SettingsTabSection {
  display(containerEl) {
    const { config } = this;
    const titleSetting = this.addSectionTitle(
      containerEl,
...
class Handler {
  ...
  getFacets(mode) {
    if (!this.facets) {
      ...
      if (facetList) {
        this.facets = Object.values(facetList).filter(
          (facet) => facet.mode === mode,
        );
      }
    }
    return this.facets ?? [];
  ...
  getSourceInfoFromSuggestion(suggestion) {
    ...
    const isFileBasedSuggestion =
      suggestion && !invalidTypes.includes(suggestion.type);
  ...
  navigateToLeafOrOpenFile(
    ...
    this.navigateToLeafOrOpenFileAsync(
      ...
      console.log(
        `Switcher++: error navigating to open file. ${errorContext}`,
        reason,
      );
    });
  }
  ...
  async navigateToLeafOrOpenFileAsync(
    ...
    const { leaf: targetLeaf } = this.findMatchingLeaf(
      file,
      leaf,
      shouldIncludeRefViews,
    );
  ...
  renderPath(
    ...
    if (parentEl && file) {
      ...
      let hidePath =
        format === PathDisplayFormat.None ||
        (isRoot && this.settings.hidePathIfRoot);
    ...
    if (file) {
      ...
      switch (displayFormat) {
        case PathDisplayFormat.FolderWithFilename:
          text = isRoot
            ? `${file.name}`
            : obsidian.normalizePath(`${dirname}/${file.name}`);
          break;
        ...
        if (matchStartIndex >= nameIndex) {
          ...
          while (i--) {
            ...
            if (matchPartEndIndex <= nameIndex) {
              ...
              pathMatch = {
                score,
                matches: matches.slice(0, nextMatchPartIndex),
              };
              basenameMatch = {
                score,
                matches: matches.slice(nextMatchPartIndex),
              };
              decrementOffsets(basenameMatch.matches, nameIndex);
              break;
  ...
  renderAsFileInfoPanel(
    ...
    if (primaryString?.length) {
      ...
      if (matchType === MatchType.Basename) {
        ...
        ({ pathMatch, basenameMatch: primaryMatch } =
          this.splitSearchMatchesAtBasename(file, match));
      }
    }
  ...
  static applyMatchPriorityPreferences(sugg, settings, metadataCache) {
    if (sugg?.match) {
      ...
      if (file && metadataCache?.isUserIgnored(file.path)) {
        ...
        const fileExtAdjustments =
          matchPriorityAdjustments.fileExtAdjustments ?? {};
        let factor = 0;
  ...
  static updateWorkspaceEnvListStatus(currentWorkspaceEnvList, sugg) {
    if (currentWorkspaceEnvList && sugg?.file) {
      const { file } = sugg;
      sugg.isOpenInEditor =
        currentWorkspaceEnvList.openWorkspaceFiles?.has(file);
  ...
  getItems() {
    ...
    if (workspaces) {
      Object.keys(workspaces).forEach((id) =>
        items.push({ id, type: "workspaceInfo" }),
      );
    }
...
class StandardExHandler extends Handler {
  ...
  addPropertiesToStandardSuggestions(inputInfo, sugg) {
    ...
    Handler.updateWorkspaceEnvListStatus(
...
class EditorHandler extends Handler {
  ...
  getSuggestions(inputInfo) {
    ...
    if (inputInfo) {
      ...
      items.forEach((item) => {
        ...
        if (hasSearchTerm) {
          result = this.fuzzySearchWithFallback(
            prepQuery,
            preferredTitle,
            file,
          );
  ...
  renderSuggestion(sugg, parentEl) {
    ...
    if (sugg) {
      const { file, matchType, match } = sugg;
      const hideBasename = [MatchType.None, MatchType.Primary].includes(
        matchType,
      );
      this.renderAsFileInfoPanel(
        parentEl,
      ...
      const traverseBookmarks = (bookmarks, path) => {
        bookmarks?.forEach((bookmark) => {
          if (BookmarksHandler.isBookmarksPluginGroupItem(bookmark)) {
            ...
            this.isFacetedWith(
              activeFacetIds,
              BOOKMARKS_FACET_ID_MAP[bookmark.type],
            )
          ) {
            const bookmarkInfo = {
              item: bookmark,
...
class HeadingsHandler extends Handler {
  ...
  getAvailableFacets(inputInfo) {
    ...
    return this.getFacets(inputInfo.mode).filter((facet) => {
      ...
      if (
        Object.prototype.hasOwnProperty.call(featureEnablementStatus, facet.id)
      ) {
  ...
  getSuggestionsForBookmarks(inputInfo, collection, activeFacetIds, options) {
    ...
    const { fileBookmarks, nonFileBookmarks } =
      inputInfo.currentWorkspaceEnvList;
    if (hasActiveFacets) {
      const isBookmarkFacetEnabled = activeFacetIds.has(
        HeadingsListFacetIds.Bookmarks,
      );
    ...
    if (file) {
      ...
      if (!isUserIgnored) {
        if (activeFacetIds.has(HeadingsListFacetIds.ExternalFiles)) {
          const externalFilesEnabled = showAttachments || showAllFileTypes;
          isIncluded =
            !coreFileExtensions.has(extension) && externalFilesEnabled;
        } else {
  ...
  addAliasSuggestions(inputInfo, suggestions, prepQuery, file) {
    ...
    if (frontMatter) {
      ...
      while (i--) {
        ...
        if (match) {
          suggestions.push(
            this.createAliasSuggestion(inputInfo, alias, file, match),
          );
        }
      }
    }
  ...
  addBookmarkSuggestion(inputInfo, suggestions, prepQuery, bookmarkInfo) {
    const result = this.fuzzySearchWithFallback(
      prepQuery,
      bookmarkInfo.bookmarkPath,
    );
    if (result.match) {
  ...
  addHeadingSuggestions(inputInfo, suggestions, prepQuery, file, allHeadings) {
    ...
    if (!allHeadings && h1) {
      isH1Matched = this.matchAndPushHeading(
        inputInfo,
        suggestions,
        prepQuery,
        file,
  ...
  matchAndPushHeading(inputInfo, suggestions, prepQuery, file, heading) {
    ...
    if (match) {
      suggestions.push(
        this.createHeadingSuggestion(inputInfo, heading, file, match),
      );
    }
    return !!match;
  }
  ...
  createAliasSuggestion(inputInfo, alias, file, match) {
    ...
    sugg = Handler.updateWorkspaceEnvListStatus(
      inputInfo.currentWorkspaceEnvList,
      sugg,
    );
  ...
  addRecentFilesSuggestions(file, inputInfo, prepQuery, collection) {
    ...
    if (match) {
      ...
      if (matchType === MatchType.Primary) {
        ...
        sugg = this.createFileSuggestion(
          inputInfo,
          file,
          match,
          matchType,
...
class SymbolHandler extends Handler {
  ...
  async getSuggestions(inputInfo) {
    ...
    if (inputInfo) {
      ...
      items.forEach((item) => {
        ...
        let match = null;
        if (hasSearchTerm) {
          match = obsidian.fuzzySearch(
            prepQuery,
            SymbolHandler.getSuggestionTextForSymbol(item),
          );
          shouldPush = !!match;
        }
        if (shouldPush) {
  ...
  getAvailableFacets(inputInfo) {
    ...
    const mdFacetIds = new Set(
      Object.values(SymbolType).filter((v) => isNaN(Number(v))),
    );
    facets.forEach((facet) => {
      const { id } = facet;
      facet.isAvailable = isCanvasFile
        ? canvasFacetIds.has(id)
        : mdFacetIds.has(id);
    });
  ...
  getSourceInfoForSymbolOperation(
    activeSuggestion,
    activeLeaf,
    isSymbolCmdPrefix,
    sessionOpts,
    ...
    const hasPrevSymbolSource =
      prevMode === Mode.SymbolList && !!prevSourceInfo;
    ...
    if (hasPrevSymbolSource) {
      ...
      activeSuggInfo.isValidSource &&
      !sessionOpts.useActiveEditorAsSource
    ) {
  ...
  async addCanvasSymbolsFromSource(file, symbolList, activeFacetIds) {
    ...
    if (Array.isArray(canvasNodes)) {
      canvasNodes.forEach((node) => {
        if (
          this.shouldIncludeSymbol(
            CANVAS_NODE_FACET_ID_MAP[node.type],
            activeFacetIds,
          )
        ) {
          symbolList.push({
  ...
  async addCalloutsFromSource(file, sectionCache, symbolList, activeFacetIds) {
    ...
    const shouldInclude = this.shouldIncludeSymbol(
      SymbolType.Callout,
      activeFacetIds,
    );
  ...
  addSymbolIndicator(symbolInfo, parentEl) {
    ...
    if (isCalloutCache(symbol)) {
      flairElClasses.push(
        ...["suggestion-flair", "callout", "callout-icon", "svg-icon"],
      );
...
class CommandHandler extends Handler {
  ...
  renderHotkeyForCommand(id, app, flairContainerEl) {
    try {
      ...
      console.log(
        "Switcher++: error rendering hotkey for command id: ",
        id,
        err,
      );
    }
...
class RelatedItemsHandler extends Handler {
  ...
  addOutgoingLinks(sourceFile, collection) {
    if (sourceFile) {
      ...
      outgoingLinks.forEach((linkCache) => {
        ...
        const destFile = metadataCache.getFirstLinkpathDest(
          destPath,
          sourceFile.path,
        );
        let info;
        if (destFile) {
          if (
            !incrementCount(destFiles.get(destFile)) &&
            destFile !== sourceFile
          ) {
            info = {
              file: destFile,
  ...
  getSourceInfo(activeSuggestion, activeLeaf, isPrefixCmd, sessionOpts) {
    ...
    // figure out if the previous operation was a symbol operation
    const hasPrevSource =
      prevMode === Mode.RelatedItemsList && !!prevSourceInfo;
    const activeEditorInfo = this.getEditorInfo(activeLeaf);
    const activeSuggInfo = this.getSuggestionInfo(activeSuggestion);
    if (
      !activeSuggInfo.isValidSource &&
...
class VaultHandler extends Handler {
  ...
  renderVaultSuggestion(sugg, parentEl) {
    ...
    const wrapperEl = contentEl.createDiv({
      cls: ["suggestion-note", "qsp-note"],
    });
    const iconEl = wrapperEl.createSpan({ cls: ["qsp-path-indicator"] });
    const pathEl = wrapperEl.createSpan({ cls: "qsp-path" });
  ...
  onChooseSuggestion(sugg, _evt) {
    ...
    if (sugg) {
      if (obsidian.Platform.isDesktop) {
        ...
        handled = electron.ipcRenderer.sendSync(
          "vault-open",
...
class ModeHandler {
  constructor(app, settings, exKeymap) {
    ...
    this.handlersByCommand = new Map([
      ...
      [
        settings.symbolListActiveEditorCommand,
        handlersByMode.get(Mode.SymbolList),
      ],
      [
        settings.relatedItemsListCommand,
        handlersByMode.get(Mode.RelatedItemsList),
    ...
    this.debouncedGetSuggestions = obsidian.debounce(
  ...
  onOpen() {
    ...
    if (settings.quickFilters?.shouldResetActiveFacets) {
      Object.values(settings.quickFilters.facetList).forEach(
        (f) => (f.isActive = false),
      );
    }
  }
  ...
  setSessionOpenMode(mode, chooser, sessionOpts) {
    ...
    if (mode !== Mode.Standard) {
      const openModeString =
        this.getHandler(mode).getCommandString(sessionOpts);
      Object.assign(this.sessionOpts, sessionOpts, { openModeString });
    }
    if (lastInputInfoByMode[mode]) {
      if (
        (mode === Mode.CommandList &&
          this.settings.preserveCommandPaletteLastInput) ||
        (mode !== Mode.CommandList &&
          this.settings.preserveQuickSwitcherLastInput)
      ) {
  ...
  updateSuggestions(query, chooser, modal) {
    ...
    const inputInfo = this.determineRunMode(
      query,
      activeSugg,
      activeLeaf,
      sessionOpts,
    ...
    if (mode !== Mode.Standard) {
      if (
        mode === Mode.HeadingsList &&
        inputInfo.parsedCommand().parsedInput?.length
      ) {
  ...
  toggleMobileCreateFileButton(modal, mode, config) {
    ...
    modal.allowCreateNewFile =
      config.allowCreateNewFileInModeNames.includes(modeName);
  ...
  removeEscapeCommandCharFromInput(inputInfo, escapeCmdChar, cmdStr) {
    const sansEscapeInput = inputInfo.inputTextSansEscapeChar.replace(
      new RegExp(
        `(?:${escapeRegExp(escapeCmdChar)})(?:${escapeRegExp(cmdStr)})`,
      ),
      cmdStr,
    );
  ...
  validatePrefixCommands(inputInfo, activeSugg, activeLeaf, config) {
    ...
    if (match) {
      ...
      cmdStr = match[2];
      if (containsNegation) {
        this.removeEscapeCommandCharFromInput(
          inputInfo,
          config.escapeCmdChar,
          cmdStr,
        );
    ...
    if (!isValidated && handler) {
      inputInfo.sessionOpts.useActiveEditorAsSource =
        activeEditorCmds.includes(cmdStr);
  ...
  validateSourcedCommands(
      ...
      while ((match = re.exec(searchText)) !== null) {
        ...
        if (containsNegation) {
          ...
          if (handler) {
            ...
            const unmatched = this.getSourcedHandlers().filter(
              (v) => v !== handler,
            );
    ...
    this.resetSourcedHandlers(
      unmatchedHandlers.length ? unmatchedHandlers : null,
    );
    return isValidated;
  }
  ...
  addWorkspaceEnvLists(inputInfo) {
    if (inputInfo) {
      ...
      const { fileBookmarks, nonFileBookmarks } = this.getHandler(
        Mode.BookmarksList,
      ).getItems(null);
...
class SwitcherPlusKeymap {
  ...
  constructor(app, scope, chooser, modal, config) {
    ...
    this.standardInstructionsEl = modal.modalEl.querySelector(
      ".prompt-instructions",
    );
  }
  initKeysInfo() {
  ...
  registerFacetBinding(scope, keymapConfig) {
    ...
    if (facets?.facetList?.length) {
      ...
      const registerFn = (modKeys, key, facetListLocal, isReset) => {
        return scope.register(modKeys, key, () =>
          onToggleFacet(facetListLocal, isReset),
        );
      };
  ...
  updateKeymapForMode(keymapConfig) {
    ...
    const customKeysToAdd = customKeymaps.filter((v) =>
      v.modes?.includes(mode),
    );
    if (mode === Mode.Standard) {
      this.updateKeymapForStandardMode(
        scope,
        customKeysToAdd,
        savedStandardKeysInfo,
      );
    ...
    this.showModeTriggerInstructions(
      modal.modalEl,
      showModeTriggerInstructions,
    );
  }
  ...
  updateKeymapForStandardMode(scope, customKeysToAdd, savedStandardKeysInfo) {
    ...
    const reregisterKeymaps = savedStandardKeysInfo.map(
      ([keymap, eventHandler]) => {
        return {
  ...
  unregisterKeys(scope, keymaps) {
    ...
    keymaps.map((keymap) => {
      ...
      const modifierStr =
        SwitcherPlusKeymap.modifiersToKeymapInfoStr(modifiers);
  ...
  renderFacetInstructions(parentEl, facetSettings, facetKeysInfo) {
    if (facetKeysInfo?.length && facetSettings.shouldShowFacetInstructions) {
      const facetInstructionsEl = this.getCustomInstructionsEl(
        "facets",
        parentEl,
      );
  ...
  renderCustomInstructions(parentEl, keymapInfo) {
    const customInstructionsEl = this.getCustomInstructionsEl(
      "custom",
      parentEl,
    );
  ...
  togglePinnedCommand(_evt, _ctx) {
    ...
    const pluginInstance =
      CommandHandler.getEnabledCommandPalettePluginInstance(app);
  ...
  insertIntoEditorAsLink(sugg, activeLeaf, insertConfig) {
    ...
    const activeMarkdownView = workspace.getActiveViewOfType(
...
function createSwitcherPlus(app, plugin) {
  const SystemSwitcherModal =
    getSystemSwitcherInstance(app)?.QuickSwitcherModal;
  ...
  if (coreLauncherButtonEl) {
    ...
    if (qspLauncherButtonEl) {
      ...
      if (iconName?.length) {
        ...
        const iconEl = qspLauncherButtonEl.querySelector(
...
class MobileLauncher {
  ...
  static installMobileLauncherOverride(app, launcherConfig, onclickListener) {
    ...
    const coreLauncherButtonEl = getCoreLauncherButtonElement(
      app,
      launcherConfig,
    );
    if (coreLauncherButtonEl) {
      ...
      if (
        replaceCoreLauncherButtonWithQSPButton(
          coreLauncherButtonEl,
          qspButtonEl,
        )
      ) {
